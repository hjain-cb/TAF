...
if [ "$server_type" = "SERVERLESS_COLUMNAR" ]; then
  cluster_info="{\"pod\": \"$capella_api_url\", \"tenant_id\": \"$tenant_id\", \"capella_user\": \"$capella_user\", \"capella_pwd\": \"$capella_password\", \"region\": \"$capella_region\", \"project\": \"$project_id\", \"instance_id\": \"$instance_id\", \"clusters\": \"$clusters\", \"override_token\": \"$override_token\", \"columnar_image\": \"$cbs_image\", \"override_key\": \"$override_key\"}"
